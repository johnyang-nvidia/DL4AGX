...
While integrating custom operators into TensorRT traditionally requires hand-written CUDA kernels, [Triton PTX/Cubin Integration for TensorRT Plugins](./triton2cubin-plugin-guidance) presents an alternative workflow using [Triton](https://triton-lang.org/)—a Python-based GPU programming language—to author kernels, compile them to [PTX/Cubin](https://developer.nvidia.com/blog/understanding-ptx-the-assembly-language-of-cuda-gpu-computing/), and load them at runtime via the [CUDA Driver API](https://docs.nvidia.com/cuda/cuda-driver-api/). BEVPoolV2 serves as the working example throughout.
